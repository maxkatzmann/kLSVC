...
<pre>
    ...
    */
    ...
    unsigned int num_vertices() const { return (unsigned int)G_.size(); }
    ...
    const vertex_set &out_neighbors(const vertex &a)const
     ...
     {
        return (unsigned int)in_neighbors(a).size();
     }

     unsigned int out_degree(const vertex &a) const
     {
        return (unsigned int)out_neighbors(a).size();
     }
    ...
    void insert_edge(iterator pa, iterator pb)
    {
      ...
      unsigned int old_size = (unsigned int)out_neighbors(pa).size();

      out_neighbors(pa).insert(b);
      in_neighbors(pb).insert(a);

      unsigned int new_size = (unsigned int)out_neighbors(pa).size();
...
inline static  unsigned int out_degree(const_iterator p) 
  {
    return (unsigned int)(p->second).second.size();
  }
...
inline static  unsigned int in_degree(const_iterator p) 
  {
    return (unsigned int)(p->second).first.size();
  }
